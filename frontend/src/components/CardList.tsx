  ...
  useEffect(() => {
    const fetchLocalIpAddress = async () => {
      try {
        ...
        setLocalIpAddress(window.location.hostname); // Fallback to current hostname if unable to fetch IP
      }
    };
  ...
  return (
    <div className="space-y-8">
      {!isLoading && cards.length === 0 ? (
        ...
        <>
          <QRCodeGenerator
            url={`${window.location.protocol}//${localIpAddress}:${window.location.port}/card-list/${listId}`}
            name={listName}
